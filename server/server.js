...
app.get("/api/v1/entries/:id", async (req, res) => {
  try {
    const results = await db.query("SELECT * FROM entries WHERE id=$1;", [
      req.params.id,
    ]);
    res.status(200).json({
      status: "SUCCESS",
...
app.post("/api/v1/entries", async (req, res) => {
  try {
    ...
    const results = await db.query(
      "INSERT INTO entries(time, location, how, personal, professional, feel) VALUES($1, $2, $3,$4,$5,$6) returning *;",
      [
        req.body.time,
        req.body.location,
        req.body.how,
...
// edit an entry
app.put("/api/v1/entries/:item_id", async (req, res) => {
  try {
    // console.log("params:", req.params);
    const result = await db.query(
      "UPDATE entries SET time=$1, location=$2, how=$3, personal=$4, professional=$5, feel=$6, goal_complete=$7 WHERE id=$8 returning *;",
      [
        req.body.time,
        req.body.location,
        req.body.how,
...
app.delete("/api/v1/entries/:id", async (req, res) => {
  try {
    const results = await db.query("DELETE FROM entries WHERE id=$1;", [
      req.params.id,
    ]);
    res.status(204).json({
      status: "SUCCESS",
